...
async function switchCamera(deviceId) {
  ...
  const constraints = {
    video: { 
      ...
      width: { ideal: 1280 },
      height: { ideal: 720 },
      facingMode: deviceId === 'default' ? 'user' : 'environment'
    }
  };
  ...
  try {
    ...
    // Mirror the video for signing (except for rear-facing cameras)
...
window.addEventListener('DOMContentLoaded', async () => {
  await getCameras();
  switchCamera('default'); // Start with default camera
...
// Modified video call function
async function startVideoCall() {
  // Pause sign recognition
  pauseSignRecognition();
  ...
  try {
    ...
    // Initialize OpenTok with the selected camera
